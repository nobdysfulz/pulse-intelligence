...
const sortByCreatedDateDesc = (items = []) =>
  [...items].sort((a, b) => new Date(b.created_date || b.createdAt).getTime() - new Date(a.created_date || a.createdAt).getTime());
...
const ContentItemCard = ({ title, description }: any) => {
...
export default function ContentStudioPage() {
  ...
  const [weeklyTopic, setWeeklyTopic] = useState<any>(null);
  const [weeklyContentPacks, setWeeklyContentPacks] = useState<any[]>([]);
  const [featuredPacks, setFeaturedPacks] = useState<any[]>([]);
  const [recentContent, setRecentContent] = useState<any[]>([]);
  const [socialMediaTemplates, setSocialMediaTemplates] = useState<any[]>([]);
  const [showCreditModal, setShowCreditModal] = useState(false);
  const [selectedContent, setSelectedContent] = useState<any>(null);
  const [showContentDetail, setShowContentDetail] = useState(false);
  const [marketIntelligence, setMarketIntelligence] = useState<any>(null);
  const [preferences, setPreferences] = useState<any>(null);
  const [preferenceRecordId, setPreferenceRecordId] = useState<any>(null);
  const [isUpdatingPrefs, setIsUpdatingPrefs] = useState(false);
  const [generatingTaskId, setGeneratingTaskId] = useState<any>(null);
  const [newlyGeneratedId, setNewlyGeneratedId] = useState<any>(null);
  ...
  const renderSidebarContent = () => {
    switch (activeTab) {
      ...
      case 'calendar':
        const handleCopyToClipboard = (text, title: any) => {
