...
const normalizeGoalRecord = (goal: any) => {
...
export default function GoalsPage() {
  ...
  const { user, goals: contextGoals, businessPlan, preferences, refreshUserData } = useContext(UserContext);

  // Create refresh batcher instance
  const refreshBatcher = useMemo(() => new RefreshBatcher(refreshUserData), [refreshUserData]);

  const [activeTab, setActiveTab] = useState('tracking');
  const [goals, setGoals] = useState<any[]>([]); // This will now hold ALL goals
  ...
  const [crmConnected, setCrmConnected] = useState<any>(null);
  ...
  const loadPageData = async () => {
    ...
    try {
      ...
      const allGoalsWithConfidence = (contextGoals || []).map((goal: any) => {
      ...
      if (user) {
        try {
          ...
          const crmConnections: string[] = [...(connections.crmConnections || []), ...(connections.externalConnections || [])]
  ...
  const activityDriversData = useMemo(() => {
    const sorted: string[] = [...activeActivityGoals].sort((a, b) => (b.targetValue || 0) - (a.targetValue || 0));
    return sorted.slice(0, 2).map((goal: any) => {
  ...
  const buildInsights = useCallback(() => {
    ...
    const analysisParts: string[] = [`Your overall goal progress is ${summaryData.overallProgress}%.`];
    if (laggingGoal) {
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          <Card className="bg-white">
            ...
            <CardContent className="space-y-4">
              {priorityGoalsData.map((goal: any) =>
                <div key={goal.id}>
          ...
          <Card className="bg-white">
            ...
            <CardContent className="space-y-4">
              {activityDriversData.map((goal: any) =>
                <div key={goal.id}>
                  <p className="text-sm font-medium">{goal.title}</p>
    ...
    if (loading) {
      return (
        <div className="flex items-center justify-center p-12">
          <InlineLoader message="Loading sidebar..." className="" />
        </div>
      );
    }
          ...
          <div className="space-y-6">
            ...
            {activeActivityGoals.length > 0 ? (
              <div className="space-y-4">
                {activeActivityGoals.map((goal: any) => {
