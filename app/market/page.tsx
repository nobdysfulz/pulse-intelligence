...
const FormattedAnalysis = ({ analysis }: any) => {
  if (!analysis) {
  ...
  const renderMetric = (label, value, change: any) => {
...
export default function MarketPage() {
  ...
  const canGenerateReport = () => {
    ...
    const now = new Date();
    const daysSinceLastGeneration = Math.floor((now.getTime() - lastGeneratedDate.getTime()) / (1000 * 60 * 60 * 24));
  ...
  const drawTable = (doc, startY, headers, data: any) => {
    let y = startY;
    const cellPadding = 3;
    const lineHeight = 8;
  ...
  const handleDownloadPDF = () => {
    ...
    try {
      ...
      const tableHeaders: string[] = ['Metric', 'Value', 'YoY Change'];
      const metricsData= [
  ...
  const handleSendMessage = async (e) => {
    ...
    const updatedHistory: string[] = [...chatMessages, userMessage];
        ...
        return (
          <div className="space-y-6">
            ...
            <div className="space-y-3">
                {marketConfig.zipCodes.map((zip: any) =>
