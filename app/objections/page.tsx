...
export default function ObjectionsPage() {
    const [objectionScripts, setObjectionScripts] = useState<any[]>([]);
    ...
    return (
        <div className="p-4 md:p-6 max-w-7xl mx-auto space-y-6">
            <div className="flex items-center justify-between">
                <Button variant="ghost" onClick={() => router.push('/role-play')}>
                ...
                {filteredObjections.map((objection) => {
                     ...
                     return (
                        <Card key={objection.id} className="relative overflow-hidden">
                            {isLocked && <LockedFeature feature="This Objection" description="Upgrade your plan to access this premium objection scenario." />}
                                ...
                                <AccordionItem value={`item-${objection.id}`} className="border-b-0">
                                    <AccordionTrigger className="p-4 font-semibold text-left hover:no-underline">
                                        <div className="flex items-center gap-4 w-full">
                                            ...
                                            <span className="text-left flex-grow">{objection.title}</span>
                                        </div>
                                    </AccordionTrigger>
                                    <AccordionContent className="p-4 pt-0">
                                        <div className="space-y-4">
                                            <div>
                                                ...
                                                <p className="text-slate-600">{objection.situation}</p>
                                            </div>
                                            <div>
                                                <h4 className="font-semibold text-pink-600 mb-1">Recommended Response</h4>
                                                <p className="text-slate-600 whitespace-pre-wrap font-mono text-sm">{objection.response}</p>
                                            </div>
                                            <div>
                                                <h4 className="font-semibold text-purple-600 mb-1">Key Tips</h4>
                                                <ul className="list-disc pl-5 space-y-1 text-slate-600">
                                                    {objection.tips.map((tip: string, index: number) =>
                                                        <li key={index}>{tip}</li>
                                                    )}
                                                </ul>
                                            </div>
