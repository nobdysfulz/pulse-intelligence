...
const parseTranscript = (transcriptData: any) => {
...
const normalizeAnalysis = (analysis: any) => {
  ...
  const resolveFeedback = (keys, fallback: any) => {
...
const ScenarioSearchSidebar = ({ scenarios, onStartScenario, isInitiating, activeScenarioId, getDifficultyColor }: any) => {
...
const ActiveSessionSidebar = ({ scenario, onEndSession }: any) => {
    ...
    const sections = [`<h2>${scenario.name}</h2>`,
      ...
      `<h3>Learning Objectives</h3><ul>${
        (scenario.learningObjectives || []).length > 0
          ? scenario.learningObjectives.map((item: any) => `<li>${item}</li>`).join('')
          : '<li>Not specified</li>'
      }</ul>`,
      `<h3>Success Criteria</h3><ul>${
        (scenario.successCriteria || []).length > 0
          ? scenario.successCriteria.map((item: any) => `<li>${item}</li>`).join('')
          : '<li>Not specified</li>'
      }</ul>`,
  ...
  const handleDownload = async () => {
    ...
    try {
      ...
      const addSection = (title, content: any) => {
        doc.setFont('helvetica', 'bold');
        doc.setFontSize(12);
        doc.text(title, 40, y);
        y += 16;
...
const SessionResultsSidebar = ({ sessionLog, scenario, analysis, onDelete }: any) => {
...
export default function RolePlayPage() {
  ...
  const [featuredScenarios, setFeaturedScenarios] = useState<any[]>([]);
  ...
  const [activeSessionScenario, setActiveSessionScenario] = useState<any>(null);
  ...
  const loadPageData = async () => {
    ...
    try {
      ...
      const mappedScenarios = scenarios.map((scenario: any) => ({
        ...scenario,
  ...
  const handleSessionLogClick = (log: any) => {
    if (activeSessionScenario) {
        ...
        <div>
          ...
          {featuredScenarios.length === 0 ? (
            ...
            <div className="flex overflow-x-auto gap-6 pb-4">
              {featuredScenarios.map((scenario: any) => (
