...
const TranscriptMessage = ({ turn }: any) => {
...
export default function SessionResultsPage() {
  ...
  const loadAudioUrl = async (fileUri: string) => {
  ...
  const loadData = async () => {
    ...
    try {
      ...
      // Fetch all scenarios
      const scenarioIds = [...new Set(userSessions.map((s: any) => s.scenarioId))] as string[];
      const scenariosMap: any = {};
      ...
      const totalTime = completedSessions.reduce((sum: number, s: any) => sum + (s.durationSeconds || 0), 0);
