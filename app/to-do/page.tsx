...
export default function ToDoPage() {
  ...
  const [calendarEvents, setCalendarEvents] = useState<any[]>([]); // This state is kept but not used by new ScheduleView
  const [selectedMonth, setSelectedMonth] = useState(new Date());
  const [displayedTasksCount, setDisplayedTasksCount] = useState(10);
  const [aiTakeaways, setAiTakeaways] = useState<any>(null);
  ...
  const monthlyGoals = useMemo(() => {
    ...
    return uniqueAnnualGoals.map((goal: any) => {
  ...
  const handleDownloadCSV = () => {
    const allTasks= [
      ...todaysTasks.map((t: any) => ({ ...t, list: 'Due Today' })),
      ...completedToday.map((t: any) => ({ ...t, list: 'Completed' })),
      ...overdueTasks.map((t: any) => ({ ...t, list: 'Overdue' }))];
    ...
    const headers: string[] = ['List', 'Title', 'Description', 'Category', 'Priority', 'Status', 'Date'];
  ...
  const analyticsData = useMemo(() => {
    ...
    // --- Chart Data ---
    const completionTrend = last7DaysInterval.map((day: any) => {
    ...
    return {
      ...
      timeOfDayCompletion: Object.entries(timeOfDayCompletion).
        filter(([, value]) => (value as number) > 0) // Only include categories with values
      ...
      <div className="space-y-6">
        <div className="flex items-center justify-between">
          ...
          <div className="flex items-center gap-2">
            ...
            <button onClick={handlePrint} className="p-2 bg-white hover:bg-[#F8FAFC] border border-[#E2E8F0] rounded transition-colors" title="Print">
  ...
  const renderAnalyticsView = () => {
    const StatCard = ({ title, value, delta }: any) => {
    ...
    const COLORS: string[] = ['#8884d8', '#82ca9d', '#ffc658', '#ff8042', '#a4de6c', '#d0ed57', '#83a6ed', '#8dd1e1'];
          ...
          <div className="mb-6">
            ...
            <ResponsiveContainer width="100%" height={150}>
              <LineChart data={analyticsData.completionTrend} margin={{ top: 5, right: 20, left: -10, bottom: 5 }}>
                ...
                <Tooltip contentStyle={{ fontSize: 12, padding: '4px 8px' }} formatter={(value) => `${(value as number).toFixed(0)}%`} />
...
function ScheduleView({ allActions }) {
  ...
  const DayButton = ({ day }: any) => {
    ...
    const categories = [...new Set(dayTasks.map((t: any) => {
    ...
    <div className="flex flex-col h-full bg-white">
      <div className="p-4 border-b border-gray-200">
        ...
        <div className="flex justify-around">
          {weekDays.map((day: any) => <DayButton key={day.toISOString()} day={day} />)}
        </div>
      </div>
