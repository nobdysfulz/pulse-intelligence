...
export default function UsageDetailsPage() {
        ...
        const fetchData = async () => {
            try {
                ...
                if (subData) {
                    ...
                    const logs = await CallLog.filter({ userEmail: user.email }, '-created_date', 50);
                    setCallLogs(logs);
                }
            } catch (error) {
    ...
    const handleToggleOverages = async (checked: boolean) => {
        if (!subscription) return;
        setSubscription((prev: any) => ({...prev, allowOverages: checked}));
        try {
