...
import { EnhancedPulseEngine, PulseMetrics } from './scoring-engine'
import { PulseDataCollector, ActivityMetrics } from './data-collector'
...
export class PulseIntelligenceService {
  ...
  // Convert ActivityMetrics to PulseMetrics by calculating derived behavioral metrics
  private convertToPulseMetrics(activity: ActivityMetrics): PulseMetrics {
    // Calculate activity volume (total activities performed)
    const activityVolume = activity.tasksCompleted +
                          activity.contactsAdded +
                          activity.appointmentsSet +
                          activity.contentGenerated

    // Calculate efficiency ratio (completed vs total tasks)
    const totalTasks = activity.tasksCompleted + activity.tasksOverdue
    const efficiencyRatio = totalTasks > 0 ? activity.tasksCompleted / totalTasks : 0

    // Calculate engagement level (composite of consistency and goal progress)
    const goalProgressRatio = activity.goalsActive > 0
      ? activity.goalsOnTrack / activity.goalsActive
      : 0
    const engagementLevel = (activity.consistencyStreak / 30) * 0.6 + goalProgressRatio * 0.4

    return {
      tasksCompleted: activity.tasksCompleted,
      tasksOverdue: activity.tasksOverdue,
      highPriorityCompleted: activity.highPriorityCompleted,
      goalsActive: activity.goalsActive,
      goalsOnTrack: activity.goalsOnTrack,
      contactsAdded: activity.contactsAdded,
      appointmentsSet: activity.appointmentsSet,
      contentGenerated: activity.contentGenerated,
      systemsUsed: activity.systemsUsed,
      consistencyStreak: activity.consistencyStreak,
      activityVolume,
      efficiencyRatio,
      engagementLevel
    }
  ...
  async calculateAndStorePulseScore(userId: string): Promise<any> {
    try {
      ...
      const activityMetrics = await this.dataCollector.collectUserMetrics(userId)

      // 2. Store metrics for historical analysis
      await this.dataCollector.storeUserMetrics(activityMetrics)

      // 3. Convert to PulseMetrics with derived behavioral metrics
      const pulseMetrics = this.convertToPulseMetrics(activityMetrics)

      // 4. Calculate enhanced PULSE score
      const pulseScore = await this.pulseEngine.calculateEnhancedPulse(userId, pulseMetrics)

      // 5. Store enhanced score
      await this.storeEnhancedPulseScore(userId, pulseScore)

      // 6. Store score history for trends
      await this.storeScoreHistory(userId, pulseScore)

      // 7. Generate and store interventions if needed
