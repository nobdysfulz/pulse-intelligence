...
serve(async (req) => {
  ...
  try {
    const { transactionId, updates, userId } = await req.json();

    if (!userId || !transactionId || !updates) {
      return new Response(
        JSON.stringify({ success: false, error: 'userId, transactionId, and updates are required' }),
    ...
    if (!response.ok) {
      ...
      console.error('[updateSkySlopeTransactionTool] SkySlope API error:', error);
      return new Response(
        JSON.stringify({ 
          success: false, 
    ...
    console.error('[updateSkySlopeTransactionTool] Error:', error);
    return new Response(
      JSON.stringify({ 
        success: false, 
